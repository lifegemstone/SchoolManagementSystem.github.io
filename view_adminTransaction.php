...
if(isset($_GET['chan'])){
	...
	$paymentChannel=$_GET['chan'];
}
$mysqli=new mysqli(SERVER_NAME,DB_USER,DB_PASSWORD,DB_NAME) or die("could not connect to the database");
$select="SELECT transaction_id,amount,paid_by,phone_number,channel,reciept_img,date FROM transaction_admindata WHERE transaction_reference='$transaction_reference'";
...
$reciept_img=$result['reciept_img'];
...
echo"<a href='#' class='btn btn-primary' id='printTransaction'><i class='fa fa-print'></i> Print </a><br> 
	<div class='card printTransaction'> <h5 class='card-header'> TRANSACTION DETAILS </h5>
		...
		echo"<a href='#' class='btn btn-primary' id='printTransaction'><i class='fa fa-print'></i> Print </a><br> 
			<div class='card printTransaction' style='width:700px;height:400px'> <h5 class='card-header'> TRANSACTION DETAILS </h5>
			<img src=scanned_docs/".$reciept_img."  width='700' height='400' alt='image' />
			</div><br><br>";
