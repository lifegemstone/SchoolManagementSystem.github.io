...
.green{background:green!important;color:white}
</style>
</head>
...
<p> ".$_SESSION['attendance_error']."</p>
</div>";
unset($_SESSION['attendance_error']);
}
if(isset($_SESSION['attendanceSuccess'])){
	echo"
<div class='alert alert-success'>
<button class='close' data-dismiss='alert' aria-hidden='true'>&times </button>
<h4 class='page-header'><i class='fa fa-check'></i> SUCCESS</h4>
<p> ".$_SESSION['attendanceSuccess']."</p>
</div>";
...
if(isset($_SESSION['username'])){
		...
		if($teacher_id == ""){
		    ...
		    echo "<span style='color:red'> No Class Has Been Assigned To Your Account Yet. Kindly Contact Your Admin!!!";
		    exit();
		}
}
?>
<h4 class="page-header text-primary"><i class="fa fa-eye"></i> VIEW ATTENDANCE</h4>
<button  type='button' class='btn btn-primary btn-xs'><a href='present_list.php' style='color:white'>Present List</a></button><button type='button' class='btn btn-success btn-xs'><a href='absent_list.php' style='color:white'>Absent List</a></button>
<h4 class="page-header text-primary"><i class="fa fa-table"></i> TAKE ATTENDANCE</h4>
<p><span style='color:red'>*</span> To take class attendance kindly select a class below from the list of classes </p>
<p><span style='color:red'>*</span> Check the present checkbox to mark as present or uncheck the box to mark as absent   </p>
<form  class="form-vertical" id='class_form' action="attendance_table.php" method="GET">
