...
<!DOCTYPE HTML>
<html>
<head><title>Compute Cognitive Report</title>
...
<li class="breadcrumb-item active">Compute Cognitive Report</li>  
</ol><br>
<h5 style="border-bottom:1px solid #eee;line-height:180%"><i class="fa fa-edit"></i> Compute Cognitive Report</h5><br>
...
if($conn->connect_error){
	die('could not connect to the database');
		...
		<div class='col-sm-12 col-md-4 col-lg-4'>
			<img src='studentImages/{$results['student_img']}' class='img-fluid rounded z-depth-2'><br><br>
		</div>
...
<?php 
	if(!isset($_SESSION['success'])){
		echo"<form method='POST' action='do_cognitive_report.php'>
...
if($conn->connect_error){
	die('could not connect to the database');
		...
		$i=1;
		$select="SELECT subject from subjects WHERE class_name='$class_name'";
		$query=$conn->query($select);
		...
		while($results=$query->fetch_array(MYSQLI_ASSOC)){
			echo"<tr>
					...
					<td><input type='text' disabled id='$i'  onfocus=custom('$i') onkeyup=check1('$i') class='form-control scores' /></td>
					<td><input type='text' disabled id='grade_firstTerm$i'  class='form-control grades'/></td>
					<td><input type='text' disabled id='remarks$i'  class='form-control remarks'/></td>
					</tr>";
...
<input type='hidden' name='firstTerm_score[]' value='' id='score' />
<input type='hidden' name='grade_firstTerm[]' value=''  id='grade'/>
<input type='hidden' name='remarks[]' value='' id='remark' />
...
<button type='submit' id='button' class='btn btn-md aqua-gradient'>SUBMIT</button>
	...
	<h5 style='border-bottom:1px solid #eee;line-height:180%'><i class='fa fa-check'></i> SUCCESS</h5>
			...
			echo"<button type='button' class='btn btn-sm btn-primary'><a href='compute_assessment_report.php?s_id=$student_id &c_name=$class_name &f_tr=$first_term' style='color:white'><i class='fa fa-angle-double-left'></i> BACK</a></button>

			<button type='button' class='btn btn-sm btn-success'><a href='compute_observation_conduct_report.php?s_id=$student_id &c_name=$class_name &f_tr=$first_term' style='color:white'> NEXT <i class='fa fa-angle-double-right'></i></a></button>";
				}
...
function check(id){
	...
	if((Number.isInteger(this_value))==false){
		alert("INVALID INPUT::ONLY INTEGERS ARE ALLOWED IN THIS FIELD");
		this_id.value="";
	}
	custom(id);
}
...
function custom(id){
	...
	if(Number(this_id.value)>=80 && Number(this_id.value)<=100){
		document.getElementById("grade_firstTerm"+id).value="A";
		document.getElementById("grade_firstTerm"+id).style['color']="blue";
	...
	if(Number(this_id.value)>=0 && Number(this_id.value)<35){
		document.getElementById("grade_firstTerm"+id).value="F";
		document.getElementById("grade_firstTerm"+id).style['color']="red";
